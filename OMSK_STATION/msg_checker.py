...
# CUTTED PARAMS
max_altitude=6000 # in meter
max_distance=6000000 # 3000km
forbidden_MSG=[6,8]

# station GPS coordinate
# noise_station_lat=55.873796 # home, lat
# noise_station_lon=37.515019 # home, lon
# noise_station_alt=100 # home, altitude

 # MOSRENTGEN LAT
#  # MOSRENTGEN LON
# noise_station_lat=55.6191241 # mosrentgen, lat
# noise_station_lon=37.4662008 # mosrentgen, lon
# noise_station_alt=50 # mosrentgen, altitude

OMSK_1=(54.988934, 73.428965, 50);
# MOSRENTGEN_2=();
# noise_station_coordinates=[MOSRENTGEN_1, MOSRENTGEN_2];

noise_station_coordinates=[OMSK_1];
...
def calc_cartesian_distance(lat1, lon1, alt1, lat2, lon2, alt2):

	R = 6376.5 *1000;

	lat_1 = degreesToRadians(lat1);
	lat_2 = degreesToRadians(lat2);
	lon_1 = degreesToRadians(lon1);
	lon_2 = degreesToRadians(lon2);

	x_1 = (R+alt1) * math.cos(lat_1)*math.cos(lon_1);
	y_1 = (R+alt1) * math.sin(lon_1) * math.cos(lat_1);
	z_1 = (R+alt1) * math.sin(lat_1);

	x_2 = (R+alt2) * math.cos(lat_2)*math.cos(lon_2);
	y_2 = (R+alt2) * math.sin(lon_2) * math.cos(lat_2);
	z_2 = (R+alt2) * math.sin(lat_2);
...
def calculate_distance(noise_station_coordinates, lat2, lon2, alt2):
	min_dist=None
	for temp_station_coordinates in noise_station_coordinates:
		# cart_1=convert_to_cartesian(*temp_station_coordinates)
		# cart_2=convert_to_cartesian(lat2, lon2, alt2)
		
		# dist=sum(map(lambda x,y: (x-y)**2, cart_1, cart_2))**0.5
		dist=calc_cartesian_distance(*temp_station_coordinates, lat2, lon2, alt2)
...
def main():
	txt_list=sorted(glob.glob('OMSK_SBS.2*'), key = lambda file: os.path.getctime(file)) # find all txt in the folders
	...
	for txt_temp in txt_list:
		try:
			...
			for string in all_strings:
				# print(string)			
				if len(string)<2: # sometimes file begin only with '\n'
					continue

				string=string.split(',')
				MSG_NUM=int(string[1])

				# check line, and recover if its bad
			...
			for temp_icao in icao_list: 
				...
				for string in icao_list[temp_icao]:
					...
					if string[5] and string[8] and string[9]: # 5 is alt, 8 is lat, 9 is lon
						...
						distance_to_station=calculate_distance(noise_station_coordinates, aircraft_lat, aircraft_lon, aircraft_alt)
						# print('distance_', string[1], distance_to_station)
			...
			for temp_icao in icao_good_list:
				...
				for string in icao_list[temp_icao]:
					...
					if MSG_NUM==1: 
						new_param[3]=string[4].rstrip() # callsign
			...
			if len(icao_good_list): # to avoid empty files
				with open(path+'/cutted_files/'+str(txt_temp)+'_cutted'+'.txt', 'w+') as new_txt:
					for temp_icao in cutted_icao_list:
						for string in cutted_icao_list[temp_icao]:
							new_txt.write(','.join(string)+',\n')
			os.remove(txt_temp)
		except  Exception as e:
			print('something wrong with file  ' + str(e) + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')))
