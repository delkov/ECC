...
def gnd_detection(icao_query, param_from_msg, last_track, temp_icao):
...
# CHANGE DIR 
path="/home/ssh_sammy/sbs_store/OMSK"
os.chdir(path)
...
def main():
	...
	txt_list=sorted(glob.glob('OMSK*.gz'))#, key = lambda file: os.path.getctime(file)) # find all txt in the folders
	for txt_temp in txt_list:
		try:
			...
			for temp_icao in icao_list:
					...
					if first_time_icao:
						...
						if time_answer and (time_py-time_answer[2] <= track_memory and time_answer[2] - time_py <= track_memory):
							...
							if type_of_flight is None:
								type_of_flight=gnd_detection(icao_query,param_from_msg,last_track,temp_icao)
							...
							# here eco SHOULD BE for aircratf
							SQL = '''
				...
				# change to 32637 for MOSCOW,  32643 for OMSK, 32652 seoul; 4326 means geoid

				SQL='''
					UPDATE omsk.tracks
					SET
					    distance_1 = ST_3DDistance(
										ST_MakePoint(
	                    ...
	                    distance_2 = ST_3DDistance(
										ST_MakePoint(
											ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        altitude), 
	                                    (SELECT geom FROM omsk.static_points WHERE name='omsk_2')
	                                    ),
	                    distance_3 = ST_3DDistance(
										ST_MakePoint(
											ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        altitude), 
	                                    (SELECT geom FROM omsk.static_points WHERE name='omsk_3')
	                                    ),
	                    distance_4 = ST_3DDistance(
										ST_MakePoint(
											ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        altitude), 
	                                    (SELECT geom FROM omsk.static_points WHERE name='omsk_4')
	                                    )
	 				WHERE track=(%s)                                              
