...
# CHANGE DIR 
path="/home/ssh_sammy/sbs_store/VNK"
os.chdir(path)
...
def main():
	...
	txt_list=sorted(glob.glob('VNK*.gz'))#, key = lambda file: os.path.getctime(file)) # find all txt in the folders
	for txt_temp in txt_list:
		...
		# except psycopg2.InterfaceError as e:
			...
			try:
				...
				print(str(e) + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')))
			print('other error, ICAO is: ', icao_py)
			if commit_is_done:
