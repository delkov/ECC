...
maximum_datetime=datetime(2022,1,1,1,1,1)
...
def gnd_detection(icao_query, param_from_msg, last_track, temp_icao, txt_temp):
	...
	if altitude_py is not None and (altitude_py < max_altitude_type_detection) and (altitude_py > min_altitude_type_detection):
		if vertical_speed_py is not None and angle_py is not None: # from MSG 4 they have to be not NULL at same time
			...
			if vertical_speed_py > 0:
				...
				# print('TAKING-OFF, angle of ', temp_icao, 'is :', angle_py)
			else:
				type_of_flight=False # False means LANDING
				# print('LANDING, angle of  ',temp_icao, 'is :', angle_py)

			if (txt_temp.startswith("VNK")):
				SQL='''
					UPDATE eco.aircraft_tracks SET (type_of_flight, vpp_angle) = (%s, %s) WHERE track=(%s);
				'''
			elif (txt_temp.startswith("OMSK")):
				SQL='''
					UPDATE omsk.aircraft_tracks SET (type_of_flight, vpp_angle) = (%s, %s) WHERE track=(%s);
				'''
...
def upd_icao_query(icao_query, param_from_msg, txt_temp):
	...
	if (txt_temp.startswith("VNK")):
		...
		SQL = '''
		INSERT INTO omsk.tracks (time_track, track, callsign, altitude, speed, angle, latitude, longitude, vertical_speed) VALUES (%s,%s,%s,%s,%s, %s,%s,%s,%s)
		ON CONFLICT (time_track,track) DO UPDATE SET callsign=EXCLUDED.callsign, altitude=EXCLUDED.altitude, speed=EXCLUDED.speed, angle=EXCLUDED.angle,  latitude=EXCLUDED.latitude, longitude=EXCLUDED.longitude,	vertical_speed= EXCLUDED.vertical_speed;
		'''
...
try:
	...
	print('connectrion is established')
...
def main():
	...
	txt_list=sorted(glob.glob('*.gz'))#, key = lambda file: os.path.getctime(file)) # find all txt in the folders
	for txt_temp in txt_list:
		# try:
		commit_is_done=False # used for rollback in case bad query
		...
		with gzip.open(txt_temp,'rt', encoding='utf-8') as txt:
		...
		for temp_icao in icao_list:
			...
			for string in icao_list[temp_icao]:
				...
				icao_py, time_py = string[0], datetime(year,month,day,hour,minute,second)					
				...
				if time_py>maximum_datetime:
					continue # skip this string. sometimes we have bug with date & projection as well.
				...
				if first_time_icao:
					...
					if (txt_temp.startswith("VNK")):
						...
						print(icao_py,time_py)
						SQL="""
						SELECT track,first_time,	last_time,callsign_last_time,altitude_last_time,speed_angle_last_time,coordinate_last_time,vert_speed_last_time,	type_of_flight  from omsk.aircraft_tracks where icao=(%s) ORDER BY case when last_time > (%s) then last_time - (%s) else (%s) - last_time end limit 1;
						"""
					...
					if time_answer and (time_py-time_answer[2] <= track_memory and time_answer[2] - time_py <= track_memory):
						...
						if (txt_temp.startswith("VNK")):
							...
							SQL="""
							(SELECT callsign,altitude,speed,angle,latitude,longitude,vertical_speed FROM omsk.tracks WHERE track=(%s)  ORDER BY case WHEN time_track > (%s) then time_track - (%s) else (%s) - time_track end LIMIT 1)
							"""
						...
						if type_of_flight is None:
							type_of_flight=gnd_detection(icao_query,param_from_msg,last_track,temp_icao, txt_temp)

						## NOW WE HAVE UPDATED PARAMS in param_from_msg
						upd_icao_query(icao_query, param_from_msg, txt_temp)
						...
						if (txt_temp.startswith("VNK")):
							...
							# HERE eco FOR aircrafts!!!
							SQL = '''
							INSERT INTO eco.aircrafts (icao) VALUES (%s) ON CONFLICT (icao) DO NOTHING; 
							INSERT INTO omsk.aircraft_tracks (icao, first_time) VALUES (%s, %s) RETURNING track;
							'''
				...
				# icao is not first time
					...
					upd_icao_query(icao_query, param_from_msg, txt_temp) # last-time will be updated at the end
			...
			if (txt_temp.startswith("VNK")):
				...
				SQL='''
				UPDATE omsk.aircraft_tracks SET (last_time, callsign_last_time, altitude_last_time, speed_angle_last_time, coordinate_last_time, vert_speed_last_time) = (%s,%s,%s,%s,%s,%s) WHERE track=(%s);
				UPDATE omsk.tracks SET (callsign) = (%s) WHERE track=(%s);
				'''
			...
			if (txt_temp.startswith("VNK")):
				SQL='''
					UPDATE eco.tracks
					SET    distance_1 = ST_3DDistance(
										ST_MakePoint(
											ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32637)),
	                                        ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32637)),
	                                        altitude), 
	                                    (SELECT geom FROM eco.static_points WHERE name='mosrentgen')
	                                    )
	 				WHERE track=(%s)                                              
				...
				SQL='''
					...
					SET    distance_1 = ST_3DDistance(
										ST_MakePoint(
											ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(longitude, latitude), 4326),32643)),
	                                        altitude), 
	                                    (SELECT geom FROM omsk.static_points WHERE name='omsk_1')
	                                    )
	 				WHERE track=(%s)                                              
					AND   latitude IS NOT NULL
					AND   longitude IS NOT NULL
		...
		# 		print(str(e) + str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')))
			# print('other error')
			# if commit_is_done:
				# connect.rollback() # sometimes erro in gps.. so cancel it
