...
def main():
	...
	SQL = '''SELECT * FROM eco.noise_to_air ORDER BY id ASC;'''
	cursor.execute(SQL)
	last_ID_list=cursor.fetchall()

	SQL = '''SELECT (time_track) FROM eco.tracks ORDER BY time_track desc LIMIT 1;'''
	cursor.execute(SQL)
	...
	if len(last_time_air)>0: # the table can be empty..
		...
		for ID in last_ID_list:
			...
			SQL='''SELECT (base_name) FROM eco.noise WHERE id = (%s)'''
			data = ID
			cursor.execute(SQL, data)
			base_name = cursor.fetchall()[0][0]

			SQL='''SELECT (time_noise) FROM eco.noise WHERE id = (%s)'''
			data = ID
			cursor.execute(SQL, data)
			...
			if last_time_air-time_noise>=minimum_time:#minimum_time: # if noise_time is greater than noise datetime by 10 sec -> fine
				...
				SQL={
					'VNK001': '''SELECT (track, distance_1, time_track)  FROM eco.tracks  WHERE time_track >= (%s)::timestamp - INTERVAL '10 seconds'    and time_track <= (%s)::timestamp + INTERVAL '10 seconds' and distance_1 IS NOT NULL ORDER BY distance_1 asc LIMIT 1;''',
					'VNK002': '''SELECT (track, distance_2, time_track)  FROM eco.tracks  WHERE time_track >= (%s)::timestamp - INTERVAL '10 seconds'    and time_track <= (%s)::timestamp + INTERVAL '10 seconds' and distance_2 IS NOT NULL ORDER BY distance_2 asc LIMIT 1;''',
					'VNK003': '''SELECT (track, distance_3, time_track)  FROM eco.tracks  WHERE time_track >= (%s)::timestamp - INTERVAL '10 seconds'    and time_track <= (%s)::timestamp + INTERVAL '10 seconds' and distance_3 IS NOT NULL ORDER BY distance_3 asc LIMIT 1;''',
					'VNK004': '''SELECT (track, distance_4, time_track)  FROM eco.tracks  WHERE time_track >= (%s)::timestamp - INTERVAL '10 seconds'    and time_track <= (%s)::timestamp + INTERVAL '10 seconds' and distance_4 IS NOT NULL ORDER BY distance_4 asc LIMIT 1;'''						
				}
				...
				if len(track_distance)==0: ##  no aircraft within 10 sec..
					SQL='''
						DELETE FROM eco.noise_to_air WHERE id = (%s);
					'''
					data=ID
					# print('NO aircraft within 10s')
				else:
					SQL='''
						UPDATE eco.noise SET (track, distance, aircraft_time) = (%s, %s, %s) WHERE id=(%s);
						DELETE FROM eco.noise_to_air WHERE id = (%s);
					'''
