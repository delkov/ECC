...
path="/home/sammy/ftp/ftp_store/VNK"
os.chdir(path)
...
def main():
	...
	try:
		noise_query=[]
		file_list=sorted(glob.glob('VNK*.txt')) # by default sort by name;	key = lambda file: os.path.getctime(file)) # find all txt in the folders
		# try:
		we_have_data=False
		...
		if len(file_list)>0:
			SQL = '''SELECT id FROM eco.noise ORDER BY id DESC LIMIT 1;'''
			cursor.execute(SQL)
		...
		for file_temp in file_list:
			...
			INSERT INTO eco.noise_to_air (id) VALUES (%s); 
			'''
